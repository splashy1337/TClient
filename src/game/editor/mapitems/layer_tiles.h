...
public:
	...
	void BrushFill(CLayer *pBrush, vec2 WorldPos);
	void BrushShape(CLayer *pBrush, int X1, int Y1, int X2, int Y2, int Kind, bool Filled, int Thickness, int NgonSides = 6);
	static void CollectShapeCells(int X1, int Y1, int X2, int Y2, int Kind, bool Filled, int Thickness, std::vector<std::pair<int, int>> &Out, int NgonSides = 6);
	void BrushFlipX() override;
	void BrushFlipY() override;
	void BrushRotate(float Amount) override;
	int BrushGrabFreehand(CLayerGroup *pBrush, const std::vector<vec2> &vPolygon);
	void BrushRotateArbitrary(const std::vector<CTile> &vOrigTiles, int OrigW, int OrigH, float Angle);
	static void CollectDonutCells(float CxWorld, float CyWorld, float OuterRx, float OuterRy, float InnerRx, float InnerRy, std::vector<std::pair<int, int>> &Out);
	void BrushDonut(CLayer *pBrush, float CxWorld, float CyWorld, float OuterRx, float OuterRy, float InnerRx, float InnerRy);
