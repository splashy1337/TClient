...
void CLayerSpeedup::BrushDraw(CLayer *pBrush, vec2 WorldPos)
{
	...
	// Symmetry mirror: recursively place the brush at mirrored world positions.
	static thread_local bool s_InMirror = false;
	if(!s_InMirror && Editor()->m_SymmetryMode != 0)
	{
		s_InMirror = true;
		const vec2 C = Editor()->m_SymmetryCenter;
		const int Mode = Editor()->m_SymmetryMode;
		const bool MH = (Mode == 1 || Mode == 3);
		const bool MV = (Mode == 2 || Mode == 3);
		const float MWx = 2.0f * C.x - WorldPos.x;
		const float MWy = 2.0f * C.y - WorldPos.y;
		if(MH)
			BrushDraw(pBrush, vec2(MWx, WorldPos.y));
		if(MV)
			BrushDraw(pBrush, vec2(WorldPos.x, MWy));
		if(MH && MV)
			BrushDraw(pBrush, vec2(MWx, MWy));
		s_InMirror = false;
	}
}
