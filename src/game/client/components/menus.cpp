...
void CMenus::RenderMenubar(CUIRect Box, IClient::EClientState ClientState)
{
	...
	if(ClientState == IClient::STATE_OFFLINE)
	{
		...
		{
			int TargetedOnline = 0;
			for(int s = 0, N = ServerBrowser()->NumSortedServers(); s < N; s++)
			{
				const CServerInfo *pInfo = ServerBrowser()->SortedGet(s);
				if(pInfo)
					for(int c = 0; c < pInfo->m_NumReceivedClients; c++)
						if(GameClient()->m_ModeratorPanel.IsTodo(pInfo->m_aClients[c].m_aName))
							TargetedOnline++;
			}
			Box.VSplitRight(4.0f, &Box, nullptr);
			CUIRect CountRect;
			Box.VSplitRight(20.0f, &Box, &CountRect);
			if(m_bTodoFilter)
				CountRect.Draw(ColorRGBA(0.3f, 0.5f, 1.0f, 0.4f), IGraphics::CORNER_ALL, 4.0f);
			const ColorRGBA CountColor = TargetedOnline > 0 ? ColorRGBA(1.0f, 0.8f, 0.15f, 1.0f) : ColorRGBA(0.5f, 0.5f, 0.5f, 1.0f);
			TextRender()->SetFontPreset(EFontPreset::ICON_FONT);
			TextRender()->TextColor(CountColor);
			Ui()->DoLabel(&CountRect, FontIcon::TRIANGLE_EXCLAMATION, 12.0f, TEXTALIGN_MC);
			TextRender()->SetFontPreset(EFontPreset::DEFAULT_FONT);
			TextRender()->TextColor(TextRender()->DefaultTextColor());
			{
				const float BadgeSize = 11.0f;
				CUIRect BadgeRect = {CountRect.x + CountRect.w - BadgeSize + 2.0f, CountRect.y - 1.0f, BadgeSize, BadgeSize};
				BadgeRect.Draw(ColorRGBA(0.85f, 0.1f, 0.1f, 1.0f), IGraphics::CORNER_ALL, BadgeSize / 2.0f);
				char aBadge[8];
				str_format(aBadge, sizeof(aBadge), "%d", TargetedOnline);
				TextRender()->TextColor(1.0f, 1.0f, 1.0f, 1.0f);
				Ui()->DoLabel(&BadgeRect, aBadge, 7.5f, TEXTALIGN_MC);
				TextRender()->TextColor(TextRender()->DefaultTextColor());
			}
			static CButtonContainer s_TodoFilterBtn;
			if(Ui()->DoButtonLogic(&s_TodoFilterBtn, 0, &CountRect, BUTTONFLAG_LEFT))
				m_bTodoFilter = !m_bTodoFilter;
			GameClient()->m_Tooltips.DoToolTip(&s_TodoFilterBtn, &CountRect, m_bTodoFilter ? Localize("Show all servers") : Localize("Show only servers with targeted players"));
			// Restore ICON_FONT: subsequent tab buttons (Home, Internet, LAN, Favorites, communities)
			// are still inside the ICON_FONT rendering block started before the right-side buttons.
			TextRender()->SetFontPreset(EFontPreset::ICON_FONT);
		}
		Box.VSplitRight(10.0f, &Box, nullptr);
