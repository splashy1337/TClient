...
void CMenus::RenderServerbrowserServerList(CUIRect View, bool &WasListboxItemActivated)
{
	...
	static SColumn s_aCols[] = {
		...
		{COL_FRIENDS, IServerBrowser::SORT_NUMFRIENDS, "", 1, 30.0f, {0}},
	...
	// Build filtered list when todo-filter is active
	std::vector<int> vFilteredIndices;
	int DisplayNumServers = NumServers;
	if(m_bTodoFilter)
	{
		for(int i = 0; i < NumServers; i++)
		{
			...
			if(!pInfo)
				continue;
			for(int ci = 0; ci < pInfo->m_NumReceivedClients; ci++)
			{
				if(GameClient()->m_ModeratorPanel.IsTodo(pInfo->m_aClients[ci].m_aName))
				{
					vFilteredIndices.push_back(i);
					break;
				}
			}
		}
		DisplayNumServers = (int)vFilteredIndices.size();
	}
	...
	s_ListBox.DoStart(ms_ListheaderHeight, DisplayNumServers, 1, 3, -1, &View, false);
	...
	for(int listIdx = 0; listIdx < DisplayNumServers; listIdx++)
	{
		int i = m_bTodoFilter ? vFilteredIndices[listIdx] : listIdx;
		...
		if(ListItem.m_Selected)
			m_SelectedIndex = i; // always store the original server index
		...
		for(const auto &Col : s_aCols)
		{
			...
			{
				// Count to-do matches up front so we know whether both icons are needed.
				int TodoCount = 0;
				for(int ci = 0; ci < pItem->m_NumReceivedClients; ci++)
				{
					if(GameClient()->m_ModeratorPanel.IsTodo(pItem->m_aClients[ci].m_aName))
						TodoCount++;
				}

				const bool HasFriend = pItem->m_FriendState != IFriends::FRIEND_NO;
				const bool HasTodo = TodoCount > 0;

				// When both icons are present split the column so they don't overlap.
				CUIRect FriendRect = Button;
				CUIRect TodoRect = Button;
				if(HasFriend && HasTodo)
					Button.VSplitMid(&FriendRect, &TodoRect, 2.0f);

				if(HasFriend)
				{
					RenderBrowserIcons(*pUiElement->Rect(UI_ELEM_FRIEND_ICON), &FriendRect, ColorRGBA(0.94f, 0.4f, 0.4f, 1.0f), TextRender()->DefaultTextOutlineColor(), FontIcon::HEART, TEXTALIGN_MC);

					if(pItem->m_FriendNum > 1)
					{
						str_format(aTemp, sizeof(aTemp), "%d", pItem->m_FriendNum);
						TextRender()->TextColor(0.94f, 0.8f, 0.8f, 1.0f);
						Ui()->DoLabel(&FriendRect, aTemp, 9.0f, TEXTALIGN_MC);
						TextRender()->TextColor(TextRender()->DefaultTextColor());
					}
				}
				...
				if(HasTodo)
				{
					RenderBrowserIcons(*pUiElement->Rect(UI_ELEM_TODO_ICON), &TodoRect, ColorRGBA(1.0f, 0.85f, 0.2f, 1.0f), TextRender()->DefaultTextOutlineColor(), FontIcon::TRIANGLE_EXCLAMATION, TEXTALIGN_MC);
				}
			}
	...
	// Map listbox index → original server index (identity when filter is off)
	int newServerIdx = (m_bTodoFilter && NewSelected >= 0 && NewSelected < (int)vFilteredIndices.size())
	                       ? vFilteredIndices[NewSelected]
	                       : NewSelected;
	if(newServerIdx != m_SelectedIndex)
	{
		m_SelectedIndex = newServerIdx;
...
void CMenus::RenderServerbrowserInfoScoreboard(CUIRect View, const CServerInfo *pSelectedServer)
{
	...
	for(int i = 0; i < pSelectedServer->m_NumReceivedClients; i++)
	{
		...
		ColorRGBA Color = PlayerBackgroundColor(CurrentClient.m_FriendState == IFriends::FRIEND_PLAYER, CurrentClient.m_FriendState == IFriends::FRIEND_CLAN, CurrentClient.m_Afk, false, false);
		if(GameClient()->m_ModeratorPanel.IsTodo(CurrentClient.m_aName))
			Color = ColorRGBA(1.0f, 0.2f, 0.2f, 0.45f);
