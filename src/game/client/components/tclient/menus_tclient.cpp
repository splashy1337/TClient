...
void CMenus::RenderSettingsModeration(CUIRect MainView)
{
	CUIRect LeftView, RightView, Button, Label;
	MainView.HSplitTop(MarginSmall, nullptr, &MainView);
	MainView.VSplitMid(&LeftView, &RightView, MarginBetweenViews);
	...
	LeftView.HSplitTop(HeadlineHeight, &Label, &LeftView);
	Ui()->DoLabel(&Label, TCLocalize("KoG Authentication"), HeadlineFontSize, TEXTALIGN_ML);
	LeftView.HSplitTop(MarginSmall, nullptr, &LeftView);

	LeftView.HSplitTop(LineSize, &Label, &LeftView);
	Ui()->DoLabel(&Label, TCLocalize("Rcon username"), LineSize * 0.8f, TEXTALIGN_ML);
	LeftView.HSplitTop(LineSize * 1.5f, &Button, &LeftView);
	static CLineInput s_RconUsername(g_Config.m_McRconUsername, sizeof(g_Config.m_McRconUsername));
	s_RconUsername.SetEmptyText(TCLocalize("username"));
	Ui()->DoEditBox(&s_RconUsername, &Button, EditBoxFontSize);
	LeftView.HSplitTop(MarginSmall, nullptr, &LeftView);

	LeftView.HSplitTop(LineSize, &Label, &LeftView);
	Ui()->DoLabel(&Label, TCLocalize("Rcon password"), LineSize * 0.8f, TEXTALIGN_ML);
	LeftView.HSplitTop(LineSize * 1.5f, &Button, &LeftView);
	static CLineInput s_RconPassword(g_Config.m_McRconPassword, sizeof(g_Config.m_McRconPassword));
	s_RconPassword.SetEmptyText(TCLocalize("password"));
	s_RconPassword.SetHidden(true);
	Ui()->DoEditBox(&s_RconPassword, &Button, EditBoxFontSize);
	LeftView.HSplitTop(MarginSmall, nullptr, &LeftView);

	LeftView.HSplitTop(LineSize, &Label, &LeftView);
	Ui()->DoLabel(&Label, TCLocalize("KoG /login code"), LineSize * 0.8f, TEXTALIGN_ML);
	LeftView.HSplitTop(LineSize * 1.5f, &Button, &LeftView);
	static CLineInput s_KogLoginCode(g_Config.m_McKogLoginCode, sizeof(g_Config.m_McKogLoginCode));
	s_KogLoginCode.SetEmptyText(TCLocalize("code"));
	s_KogLoginCode.SetHidden(true);
	Ui()->DoEditBox(&s_KogLoginCode, &Button, EditBoxFontSize);
	LeftView.HSplitTop(MarginSmall, nullptr, &LeftView);

	LeftView.HSplitTop(LineSize * 2.0f, &Label, &LeftView);
	Ui()->DoLabel(&Label, TCLocalize("Auto-login runs only on servers whose name starts with |*KoG*| or [A] |*KoG*|."), LineSize * 0.7f, TEXTALIGN_ML);

	RightView.HSplitTop(HeadlineHeight, &Label, &RightView);
	Ui()->DoLabel(&Label, TCLocalize("Evidence"), HeadlineFontSize, TEXTALIGN_ML);
	RightView.HSplitTop(MarginSmall, nullptr, &RightView);

	RightView.HSplitTop(LineSize, &Label, &RightView);
	Ui()->DoLabel(&Label, TCLocalize("Attachment download directory"), LineSize * 0.8f, TEXTALIGN_ML);
	RightView.HSplitTop(LineSize * 1.5f, &Button, &RightView);
	static CLineInput s_AttachmentDir(g_Config.m_McAttachmentDir, sizeof(g_Config.m_McAttachmentDir));
	s_AttachmentDir.SetEmptyText(TCLocalize("empty = Moderator Client config dir"));
	Ui()->DoEditBox(&s_AttachmentDir, &Button, EditBoxFontSize);
	RightView.HSplitTop(MarginSmall, nullptr, &RightView);
	...
	DoButton_CheckBoxAutoVMarginAndSet(&g_Config.m_McAutoStartDiscordBridge,
		TCLocalize("Auto-start bridge on Moderator Client launch (pythonw.exe, hidden)"),
		&g_Config.m_McAutoStartDiscordBridge, &RightView, LineSize);
	RightView.HSplitTop(MarginSmall, nullptr, &RightView);
	...
	// --- Chat Watcher ---
	LeftView.HSplitTop(HeadlineHeight, &Label, &LeftView);
	Ui()->DoLabel(&Label, TCLocalize("Chat Watcher"), HeadlineFontSize, TEXTALIGN_ML);
	LeftView.HSplitTop(MarginSmall, nullptr, &LeftView);

	DoButton_CheckBoxAutoVMarginAndSet(&g_Config.m_McChatWatcher,
		TCLocalize("Scan incoming chat on KoG servers for flagged words"),
		&g_Config.m_McChatWatcher, &LeftView, LineSize);
	LeftView.HSplitTop(MarginSmall, nullptr, &LeftView);

	LeftView.HSplitTop(LineSize * 2.5f, &Label, &LeftView);
	char aCountBuf[192];
	str_format(aCountBuf, sizeof(aCountBuf),
		"Words loaded: %d  —  own messages and non-KoG servers are never scanned.\nFile: chat_watcher_wordlist.txt in config dir.",
		(int)GameClient()->m_ChatWatcher.Words().size());
	Ui()->DoLabel(&Label, aCountBuf, LineSize * 0.75f, TEXTALIGN_ML);

	LeftView.HSplitTop(LineSize, &Label, &LeftView);
	Ui()->DoLabel(&Label, TCLocalize("Add word"), LineSize * 0.8f, TEXTALIGN_ML);
	LeftView.HSplitTop(LineSize * 1.5f, &Button, &LeftView);
	CUIRect AddField, AddButton;
	Button.VSplitRight(Button.w * 0.28f, &AddField, &AddButton);
	AddField.VSplitRight(MarginSmall, &AddField, nullptr);
	static char s_aAddBuf[64] = "";
	static CLineInput s_AddInput(s_aAddBuf, sizeof(s_aAddBuf));
	s_AddInput.SetEmptyText(TCLocalize("word or phrase"));
	Ui()->DoEditBox(&s_AddInput, &AddField, EditBoxFontSize);
	static CButtonContainer s_AddButton;
	if(DoButtonLineSize_Menu(&s_AddButton, TCLocalize("Add"), 0, &AddButton, LineSize, false, 0, IGraphics::CORNER_ALL, 5.0f, 0.0f, ColorRGBA(0.0f, 0.0f, 0.0f, 0.25f)))
	{
		if(s_aAddBuf[0] != '\0')
		{
			GameClient()->m_ChatWatcher.AddWord(s_aAddBuf);
			s_aAddBuf[0] = '\0';
		}
	}
	LeftView.HSplitTop(MarginSmall, nullptr, &LeftView);
	...
	LeftView.HSplitTop(LineSize * 1.5f, &Button, &LeftView);
	CUIRect ReloadBtn, RemoveHintRight;
	Button.VSplitMid(&ReloadBtn, &RemoveHintRight, MarginSmall);
	static CButtonContainer s_ReloadWordlist;
	if(DoButtonLineSize_Menu(&s_ReloadWordlist, TCLocalize("Reload wordlist from file"), 0, &ReloadBtn, LineSize, false, 0, IGraphics::CORNER_ALL, 5.0f, 0.0f, ColorRGBA(0.0f, 0.0f, 0.0f, 0.25f)))
		Console()->ExecuteLine("mc_watcher_reload", IConsole::CLIENT_ID_UNSPECIFIED);
	Ui()->DoLabel(&RemoveHintRight, TCLocalize("Remove via console: mc_watcher_remove <word>"), LineSize * 0.7f, TEXTALIGN_ML);
}
...
void CMenus::RenderSettingsTClientInfo(CUIRect MainView)
{
	...
	RightView.HSplitTop(HeadlineHeight, &Label, &RightView);
	Ui()->DoLabel(&Label, TCLocalize("Moderator Client Developer"), HeadlineFontSize, TEXTALIGN_ML);
	RightView.HSplitTop(MarginSmall, nullptr, &RightView);
	RightView.HSplitTop(MarginSmall, nullptr, &RightView);
	...
	CUIRect TeeRect, DevCardRect;
	{
		...
		Ui()->DoLabel(&Label, "Splashy", LineSize, TEXTALIGN_ML);
